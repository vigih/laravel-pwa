...
<div {{ $attributes->merge(['class' => "flex justify-center flex-col pointer-events-none"]) }}>
    
    <div x-data="{ show: true }" 
        x-init="setTimeout(() => show = false, @js($timeout))" 
        x-show="show" 
        x-transition 
        class="fixed left-1 right-1 bottom-1 pointer-events-auto p-4 rounded-md shadow-xl flex items-center gap-2 {{ $variantClass }}"
        >
