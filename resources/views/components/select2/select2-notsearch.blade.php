{{-- resources/views/components/select2/select2-notsearch.blade.php --}}
    ...
    <div class="form-group textfield textfield-outline textfield-floating-label textfield-{{ $color }}">
        <div class="textfield-outline-wrapper">
            ...
            <select
                ...
                class="form-control select2-basic select2 select2-floating-label {{ $attributes->get('class') }}" {{
                ...
                @foreach($dataOptions as $value => $label)
                    <option
                        ...
                        {{ isset($label['disabled']) &&
                            $label['disabled'] ? 'disabled' : '' }}>
                        {{ is_array($label) ? $label['text'] : $label }}
                    </option>
                @endforeach
